...
#'   on in `args`.
#'
#'   If `args` contains a value `--verbose` the return vector is printed
#'   to the console unless `quietly` is set to TRUE.
#'
#' @param args Character vector in form of `--<parameter>`  or
#'   `--<parameter>=<value>`, usually derived by calling commandArgs()
...
#'
#' @export
process_args <- function(args, required_args,
                         optional_value_args, optional_flag_args,
                         ignored_args = c("no-echo", "no-restore", "file", "args"),
                         quietly = FALSE)
{

  required_args <- c("source-file", "output-file", "title-id", "version-id")
  # these args must have a value after '='
  optional_value_args <-
    c("characters", "genres", "point-of-view", "language", "assets")
  # these args may have a value but don't need one
  optional_flag_args <-
    c("throw-error", "throw-warnings", "sample-data", "verbose", "scanned-book")
  ignored_args <- c("no-echo", "no-restore", "file", "args")
  ...
  invalid_args <- names(args)[!names(args) %in% c(required_args, optional_args, ignored_args)]
  ...
  for(arg_name in c(ignored_args, optional_args)) {
